...
#pragma HLS INTERFACE m_axi port = output_mem depth = 1500
	...
	int ret_size;
	uint16_t eth_type_or_len = input_stream[12] & 0x00FF;
	eth_type_or_len = eth_type_or_len << 8;
	eth_type_or_len = eth_type_or_len & 0xFF00;
	eth_type_or_len += input_stream[13];

	if( eth_type_or_len<=1500 || eth_type_or_len==0x0600 )
	{	// ipv4 packet
